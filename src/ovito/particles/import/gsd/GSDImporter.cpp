...
void GSDImporter::FrameLoader::parseConvexPolyhedronShape(int typeId, QJsonObject definition, const QByteArray& shapeSpecString)
{
	...
	SurfaceMeshAccess mesh(DataOORef<SurfaceMesh>::create(dataset(), ObjectCreationParams::WithoutVisElement));
	...
	if(roundingRadius > 0) {
		SurfaceMeshAccess roundedMesh(DataOORef<SurfaceMesh>::create(dataset(), ObjectCreationParams::WithoutVisElement));
...
void GSDImporter::FrameLoader::parseSphereUnionShape(int typeId, QJsonObject definition, const QByteArray& shapeSpecString)
{
	...
	DataOORef<TriMeshObject> sphereTemplate = DataOORef<TriMeshObject>::create(dataset(), ObjectCreationParams::WithoutVisElement);
