...
void main()
{
	...
	vec3 P = view_particle_matrix_fs * (ray_origin - particle_view_pos_fs);
	vec3 D = view_particle_matrix_fs * ray_dir;
	float len = length(D);
	D /= len;
	...
	if(is_perspective) {
		...
		if(t1 < DEPTH_TOLERANCE) {
			...
			ray_origin -= ray_dir * ((DEPTH_TOLERANCE - t1) / len);
	...
	if(tnear == BOUND_HUGE || (is_perspective && tnear < 0.0)) {
		discard;
	}
