...
******************************************************************************/
void GrainSegmentationModifier::initializeObject(Application::ExecutionContext executionContext)
{
...
******************************************************************************/
Future<AsynchronousModifier::EnginePtr> GrainSegmentationModifier::createEngine(const PipelineEvaluationRequest& request, ModifierApplication* modApp, const PipelineFlowState& input, Application::ExecutionContext executionContext)
{
	// Get modifier input.
...
void GrainSegmentationEngine2::applyResults(TimePoint time, ModifierApplication* modApp, PipelineFlowState& state)
{
	...
	if(atomClusters()) {
		...
		if(modifier->colorParticlesByGrain()) {
			...
			PropertyAccess<Color> particleColorsArray = particles->createProperty(ParticlesObject::ColorProperty, false, Application::instance()->executionContext());
	...
	DataTable* grainTable = state.createObject<DataTable>(QStringLiteral("grains"), modApp, Application::instance()->executionContext(), DataTable::Scatter, GrainSegmentationModifier::tr("Grain list"), _grainSizes, _grainIds);
