...
private:
	...
	public:
		...
		CorrelationAnalysisEngine(const ModifierEvaluationRequest& request,
			...
			_neighCorrelation(doComputeNeighCorrelation ? DataTable::OOClass().createUserProperty(request.dataset(), numberOfNeighBins, PropertyObject::Float, 1, tr("Neighbor C(r)"), DataBuffer::InitializeMemory, DataTable::YProperty) : nullptr) {}
